...
public interface DashBoardRepository extends JpaRepository<__sms_report__dashboard, Integer> {
   ...
   int weekCampaignCountById(@Param("partyId") String partyId, LocalDateTime startDate, LocalDateTime endDate);

    @Query("select count(ct) from campaign ct where ct.partyId = ?1 and ct.createdTxStamp>= ?2 and ct.createdTxStamp<= ?3")
    int todaysCampaignCountById(@Param("partyId") String partyId, LocalDateTime startDate, LocalDateTime endDate);
    ...
    int todaysTotalTaskCountById(@Param("partyId") String partyId, LocalDateTime startDate, LocalDateTime endDate);
    ...
    int weekTotalTaskCountById(@Param("partyId") String partyId, LocalDateTime weekStartDate, LocalDateTime weekEndDate);
    ...
    int todaysTotalSuccessCountById(@Param("partyId") String partyId, LocalDateTime startDate, LocalDateTime endDate);
    ...
    int weekTotalSuccessCountById(@Param("partyId") String partyId, LocalDateTime weekStartDate, LocalDateTime weekEndDate);

    @Query("select ct.routeId as rId, count(ct) as cnt from campaign_task ct where campaignId in (select c.campaignId as cmpId from campaign c where c.partyId= ?1) group by routeId ")
    List<Object[]> routeDetails(@Param("partyId") String partyId);
}
