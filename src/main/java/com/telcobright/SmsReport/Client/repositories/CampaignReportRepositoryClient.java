...
List<Object[]> campaignWiseReports(String campaignId);
    ...
    @Query(value = "select campaign_id, route_id, SUM(total) as total,SUM(delivered) as delivered, SUM(inProcess) as inProcess, SUM(failed) as failed, SUM(sent) as sent\n" +
            "from\n" +
            "\t(\n" +
            "\t\tselect ct.CAMPAIGN_ID ,route_id, count(*)  as total, (select 0) as failed, (select 0) as delivered, (select 0) as inProcess, (select 0) as sent\n" +
            "\t\tfrom campaign_task ct\n" +
...
//        "\t\tand route_id = \"grameenphone\"\n" +
            ...
            "\t\tunion all\n" +
            "\t\tselect ct.CAMPAIGN_ID ,route_id, (select 0)  as total, (select 0) as failed, count(*) as delivered, (select 0) as inProcess, (select 0) as sent\n" +
            "\t\tfrom campaign_task ct \n" +
            "\t\tjoin campaign c2 on ct.CAMPAIGN_ID = c2.CAMPAIGN_ID \n" +
            "\t\twhere ct.PARENT_ID  is null\n" +
            "\t\tand (ct.last_updated_tx_stamp >= ?4 and ct.last_updated_tx_stamp <= ?5)\n" +
...
//        "\t\tand route_id = \"grameenphone\"\n" +
            ...
            "\t\tunion all\n" +
            "\t\tselect ct.CAMPAIGN_ID ,route_id, (select 0)  as total,(select 0) as failed, (select 0) as delivered, count(*) as inProcess, (select 0) as sent\n" +
            "\t\tfrom campaign_task ct \n" +
            "\t\tjoin campaign c2 on ct.CAMPAIGN_ID = c2.CAMPAIGN_ID \n" +
            "\t\twhere ct.PARENT_ID  is null\n" +
            "\t\tand (ct.last_updated_tx_stamp >= ?4 and ct.last_updated_tx_stamp <= ?5)\n" +
...
//        "\t\tand route_id = \"grameenphone\"\n" +
            ...
            "\t\tunion all\n" +
            "\t\tselect ct.CAMPAIGN_ID ,route_id,(select 0)  as total,count(*) as failed, (select 0) as delivered,(select 0) as inProcess, (select 0) as sent\n" +
            "\t\tfrom campaign_task ct \n" +
            "\t\tjoin campaign c2 on ct.CAMPAIGN_ID = c2.CAMPAIGN_ID \n" +
            "\t\twhere ct.PARENT_ID  is null\n" +
            "\t\tand (ct.last_updated_tx_stamp >= ?4 and ct.last_updated_tx_stamp <= ?5)\n" +
...
//        "\t\tand route_id = \"grameenphone\"\n" +
            ...
            "\t\tunion all\n" +
            "\t\tselect ct.CAMPAIGN_ID ,route_id ,(select 0)  as total, (select 0) as failed, (select 0) as delivered,(select 0) as inProcess,count(*)  as sent\n" +
            "\t\tfrom campaign_task ct \n" +
            "\t\tjoin campaign c2 on ct.CAMPAIGN_ID = c2.CAMPAIGN_ID \n" +
            "\t\twhere ct.PARENT_ID  is null\n" +
            "\t\tand (ct.last_updated_tx_stamp >= ?4 and ct.last_updated_tx_stamp <= ?5)\n" +
...
//        "\t\tand route_id = \"grameenphone\"\n" +
    ...
    @Query(value = "select CAMPAIGN_ID , SUM(total) as total,SUM(delivered) as delivered, SUM(inProcess) as inProcess, SUM(failed) as failed, SUM(sent) as sent\n" +
            "from\n" +
            "\t(\n" +
            "\t\tselect ct.CAMPAIGN_ID , count(*)  as total, (select 0) as failed, (select 0) as delivered, (select 0) as inProcess, (select 0) as sent\n" +
            "\t\tfrom campaign_task ct \n" +
            "\t\tjoin campaign c2 on ct.CAMPAIGN_ID = c2.CAMPAIGN_ID \n" +
            "\t\twhere ct.PARENT_ID  is null \n" +
...
//                "\t\tand (route_id = ?1 or ?1 = ' ' or ?1 = null)\n" +
            ...
            "\t\tgroup by ct.CAMPAIGN_ID #TOTAL\n" +
            "\t\tunion all\n" +
            "\t\tselect ct.CAMPAIGN_ID , (select 0)  as total, (select 0) as failed, count(*) as delivered, (select 0) as inProcess, (select 0) as sent\n" +
            "\t\tfrom campaign_task ct \n" +
            "\t\tjoin campaign c2 on ct.CAMPAIGN_ID = c2.CAMPAIGN_ID \n" +
            "\t\twhere ct.PARENT_ID  is null\n" +
            "\t\tand (ct.last_updated_tx_stamp >= ?3 and ct.last_updated_tx_stamp <= ?4)\n" +
...
//                "\t\tand (route_id = ?1 or ?1 = ' ' or ?1 = null)\n" +
            ...
            "\t\tgroup by ct.CAMPAIGN_ID #DELIVERED\n" +
            "\t\tunion all\n" +
            "\t\tselect ct.CAMPAIGN_ID , (select 0)  as total,(select 0) as failed, (select 0) as delivered, count(*) as inProcess, (select 0) as sent\n" +
            "\t\tfrom campaign_task ct \n" +
            "\t\tjoin campaign c2 on ct.CAMPAIGN_ID = c2.CAMPAIGN_ID \n" +
            "\t\twhere ct.PARENT_ID  is null\n" +
            "\t\tand (ct.last_updated_tx_stamp >= ?3 and ct.last_updated_tx_stamp <= ?4)\n" +
...
//                "\t\tand (route_id = ?1  or ?1 = ' ' or ?1 = null)\n" +
            ...
            "\t\tgroup by ct.CAMPAIGN_ID #inprocess\n" +
            "\t\tunion all\n" +
            "\t\tselect ct.CAMPAIGN_ID , (select 0)  as total,count(*) as failed, (select 0) as delivered,(select 0) as inProcess, (select 0) as sent\n" +
            "\t\tfrom campaign_task ct \n" +
            "\t\tjoin campaign c2 on ct.CAMPAIGN_ID = c2.CAMPAIGN_ID \n" +
            "\t\twhere ct.PARENT_ID  is null\n" +
            "\t\tand (ct.last_updated_tx_stamp >= ?3 and ct.last_updated_tx_stamp <= ?4)\n" +
...
//                "\t\tand (route_id = ?1 or ?1 = ' ' or ?1 = null)\n" +
            ...
            "\t\tgroup by ct.CAMPAIGN_ID #failed\n" +
            "\t\tunion all\n" +
            "\t\tselect ct.CAMPAIGN_ID , (select 0)  as total, (select 0) as failed, (select 0) as delivered,(select 0) as inProcess, count(*) as sent\n" +
            "\t\tfrom campaign_task ct \n" +
            "\t\tjoin campaign c2 on ct.CAMPAIGN_ID = c2.CAMPAIGN_ID \n" +
            "\t\twhere ct.PARENT_ID  is null\n" +
            "\t\tand (ct.last_updated_tx_stamp >= ?3 and ct.last_updated_tx_stamp <= ?4)\n" +
...
//                "\t\tand (route_id = ?1 or ?1 = ' ' or ?1 = null)\n" +
    ...
    @Query(value = "select ROUTE_ID , SUM(total) as total,SUM(delivered) as delivered, SUM(inProcess) as inProcess, SUM(failed) as failed, SUM(sent) as sent\n" +
            "from\n" +
            "\t(\n" +
            "\t\tselect ct.ROUTE_ID , count(*)  as total, (select 0) as failed, (select 0) as delivered, (select 0) as inProcess, (select 0) as sent\n" +
            "\t\tfrom campaign_task ct \n" +
            "\t\tjoin campaign c2 on ct.CAMPAIGN_ID = c2.CAMPAIGN_ID \n" +
            "\t\twhere ct.PARENT_ID  is null \n" +
            "\t\tand (ct.last_updated_tx_stamp >= ?3 and ct.last_updated_tx_stamp <= ?4)\n" +
            ...
            "\t\tunion all\n" +
            "\t\tselect ct.ROUTE_ID , (select 0)  as total, (select 0) as failed, count(*) as delivered, (select 0) as inProcess, (select 0) as sent\n" +
            "\t\tfrom campaign_task ct \n" +
            "\t\tjoin campaign c2 on ct.CAMPAIGN_ID = c2.CAMPAIGN_ID \n" +
            "\t\twhere ct.PARENT_ID  is null\n" +
            "\t\tand (ct.last_updated_tx_stamp >= ?3 and ct.last_updated_tx_stamp <= ?4)\n" +
            ...
            "\t\tunion all\n" +
            "\t\tselect ct.ROUTE_ID , (select 0)  as total,(select 0) as failed, (select 0) as delivered, count(*) as inProcess, (select 0) as sent\n" +
            "\t\tfrom campaign_task ct \n" +
            "\t\tjoin campaign c2 on ct.CAMPAIGN_ID = c2.CAMPAIGN_ID \n" +
            "\t\twhere ct.PARENT_ID  is null\n" +
            "\t\tand (ct.last_updated_tx_stamp >= ?3 and ct.last_updated_tx_stamp <= ?4)\n" +
            ...
            "\t\tunion all\n" +
            "\t\tselect ct.ROUTE_ID , (select 0)  as total,count(*) as failed, (select 0) as delivered,(select 0) as inProcess, (select 0) as sent\n" +
            "\t\tfrom campaign_task ct \n" +
            "\t\tjoin campaign c2 on ct.CAMPAIGN_ID = c2.CAMPAIGN_ID \n" +
            "\t\twhere ct.PARENT_ID  is null\n" +
            "\t\tand (ct.last_updated_tx_stamp >= ?3 and ct.last_updated_tx_stamp <= ?4)\n" +
            ...
            "\t\tunion all\n" +
            "\t\tselect ct.ROUTE_ID , (select 0)  as total, (select 0) as failed, (select 0) as delivered,(select 0) as inProcess, count(*) as sent\n" +
            "\t\tfrom campaign_task ct \n" +
            "\t\tjoin campaign c2 on ct.CAMPAIGN_ID = c2.CAMPAIGN_ID \n" +
            "\t\twhere ct.PARENT_ID  is null\n" +
            "\t\tand (ct.last_updated_tx_stamp >= ?3 and ct.last_updated_tx_stamp <= ?4)\n" +
